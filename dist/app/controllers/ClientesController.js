...
class ClientesController {
  ...
  async index(req, res) {
    try {
      ...
      if (req.userRole === 'admin') {
        const clientes = await _clientejs2.default.findAll();
        return res.json(clientes);
      }

      // Se for cliente comum, retorna apenas o próprio perfil
      const cliente = await _clientejs2.default.findByPk(req.userId);
      if (!cliente) {
  ...
  async show(req, res) {
    try {
      ...
      const cliente = await _clientejs2.default.findByPk(id);
  ...
  async create(req, res) {
    ...
    try {
      ...
      const novoCliente = await _clientejs2.default.create({ nome, email, telefone, senha });
...
async update(req, res) {
  ...
  try {
    ...
    const { nome, email, telefone, senha } = req.body;
    const cliente = await _clientejs2.default.findByPk(userId);
