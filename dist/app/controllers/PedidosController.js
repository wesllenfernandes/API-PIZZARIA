"use strict";Object.defineProperty(exports, "__esModule", {value: true}); function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; } function _nullishCoalesce(lhs, rhsFn) { if (lhs != null) { return lhs; } else { return rhsFn(); } }var _pedidojs = require('../../models/pedido.js'); var _pedidojs2 = _interopRequireDefault(_pedidojs);
var _clientejs = require('../../models/cliente.js'); var _clientejs2 = _interopRequireDefault(_clientejs);
var _Pizzajs = require('../../models/Pizza.js'); var _Pizzajs2 = _interopRequireDefault(_Pizzajs);
...
async index(req, res) {
    ...
    if (userRole === 'admin') {
      ...
      pedidos = await _pedidojs2.default.findAll();
    } else {
 ...
 /* async listarPedidosCliente(req, res) {
  ...
  async show(req, res) {
    try {
      ...
      const pedido = await _pedidojs2.default.findByPk(id);
  ...
  try {
    ...
    // Verifica se o cliente existe
    const cliente = await _clientejs2.default.findByPk(id_cliente);
    if (!cliente) {
      return res.status(404).json({ error: 'Cliente não encontrado' });
    }

    // Verifica se a pizza existe
    const pizza = await _Pizzajs2.default.findByPk(id_pizza);
    if (!pizza) {
      return res.status(404).json({ error: 'Pizza não encontrada' });
    }

    // Cria o pedido
    const novoPedido = await _pedidojs2.default.create({
      id_cliente,
      id_pizza,
      forma_pagamento,
...
async update(req, res) {
  ...
  try {
    ...
    const pedido = await _pedidojs2.default.findByPk(id_pedido);
    ...
    pedido.id_pizza = _nullishCoalesce(id_pizza, () => ( pedido.id_pizza));
    pedido.status = _nullishCoalesce(status, () => ( pedido.status));
    pedido.forma_pagamento = _nullishCoalesce(forma_pagamento, () => ( pedido.forma_pagamento));
    pedido.total_pedido = _nullishCoalesce(total_pedido, () => ( pedido.total_pedido));
    pedido.prioridade = _nullishCoalesce(prioridade, () => ( pedido.prioridade));
